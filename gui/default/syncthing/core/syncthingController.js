  ...
  .controller(
    ...
    function (
      ...
      function updateLocalConfig(config) {
        ...
        $scope.config = config;
        $scope.config.options._listenAddressesStr =
          $scope.config.options.listenAddresses.join(", ");
        $scope.config.options._globalAnnounceServersStr =
          $scope.config.options.globalAnnounceServers.join(", ");
      ...
      function refreshCompletion(device, folder) {
        ...
        $http
          .get(
            urlbase + "/db/completion?device=" + device + "&folder=" + encodeURIComponent(folder)
          )
          .success(function (data) {
          ...
          .success(function (data) {
            ...
            try {
              data.total.inbps = Math.max(
                0,
                (data.total.inBytesTotal - $scope.connectionsTotal.inBytesTotal) / td
              );
              data.total.outbps = Math.max(
                0,
                (data.total.outBytesTotal - $scope.connectionsTotal.outBytesTotal) / td
              );
            } catch (e) {
              data.total.inbps = 0;
              data.total.outbps = 0;
            ...
            for (id in data) {
              ...
              try {
                data[id].inbps = Math.max(
                  0,
                  (data[id].inBytesTotal - $scope.connections[id].inBytesTotal) / td
                );
                data[id].outbps = Math.max(
                  0,
                  (data[id].outBytesTotal - $scope.connections[id].outBytesTotal) / td
                );
              } catch (e) {
                data[id].inbps = 0;
                data[id].outbps = 0;
      ...
      function shouldSetDefaultFolderPath() {
        return (
          $scope.config.defaults.folder.path &&
          $scope.folderEditor.folderPath.$pristine &&
          $scope.editingFolderNew()
        );
      }
        ...
        $http
          ...
          .success(function (data) {
            ...
            for (var device in $scope.deviceStats) {
              ...
              if (
                $scope.deviceStats[device].lastSeen.toISOString() !== "1970-01-01T00:00:00.000Z"
              ) {
      ...
      $scope.syncPercentage = function (folder) {
        ...
        if (
          ($scope.model[folder].needBytes == 0 && $scope.model[folder].needDeletes > 0) ||
          $scope.model[folder].globalBytes == 0
        ) {
        ...
        return progressIntegerPercentage(
          $scope.model[folder].inSyncBytes,
      ...
      $scope.scanPercentage = function (folder) {
        ...
        return progressIntegerPercentage(
          $scope.scanProgress[folder].current,
      ...
      $scope.remoteGUIAddress = function (deviceCfg) {
        ...
        return (
          "http://" +
          replaceAddressPort(conn.address, deviceCfg.remoteGUIPort).replace("%.*?]:", "]:")
        );
      };
      ...
      $scope.logging = {
        ...
        fetch: function () {
          ...
          $http
            .get(urlbase + "/system/log" + (last ? "?since=" + encodeURIComponent(last) : ""))
            .success(function (data) {
              if (!$scope.logging.timer) return;
        ...
        $scope.deviceFolders($scope.currentDevice).forEach(function (folderID) {
          ...
          for (var i = 0; i < folderdevices.length; i++) {
            if (folderdevices[i].deviceID === deviceCfg.deviceID) {
              $scope.currentSharing.encryptionPasswords[folderID] =
                folderdevices[i].encryptionPassword;
              break;
            }
          }
        });
      ...
      $scope.saveDevice = function () {
        $scope.currentDevice.addresses = $scope.currentDevice._addressesStr
          .split(",")
          .map(function (x) {
            return x.trim();
          });
          ...
          if ($scope.currentSharing.selected[id]) {
            ...
            for (i = 0; i < $scope.folders[id].devices.length; i++) {
              if ($scope.folders[id].devices[i].deviceID === currentID) {
                ...
                $scope.folders[id].devices[i].encryptionPassword =
                  $scope.currentSharing.encryptionPasswords[id];
                break;
              }
            }
      ...
      $scope.unignoreDeviceFromTemporaryConfig = function (ignoredDevice) {
        $scope.tmpRemoteIgnoredDevices = $scope.tmpRemoteIgnoredDevices.filter(function (
          existingIgnoredDevice
        ) {
      ...
      $scope.unignoreFolderFromTemporaryConfig = function (device, ignoredFolderID) {
        for (var i = 0; i < $scope.tmpDevices.length; i++) {
          if ($scope.tmpDevices[i].deviceID == device) {
            $scope.tmpDevices[i].ignoredFolders = $scope.tmpDevices[i].ignoredFolders.filter(
              function (existingIgnoredFolder) {
      ...
      function initVersioningEditing() {
        ...
        switch (currentVersioning.type) {
          case "trashcan":
            $scope.currentFolder._guiVersioning.trashcanClean =
              +currentVersioning.params.cleanoutDays;
            break;
          case "simple":
            ...
            break;
          case "staggered":
            $scope.currentFolder._guiVersioning.staggeredMaxAge = Math.floor(
              +currentVersioning.params.maxAge / 86400
            );
            break;
          case "external":
      ...
      function editFolderGetIgnores() {
        return $http
          .get(urlbase + "/db/ignores?folder=" + encodeURIComponent($scope.currentFolder.id))
          .then(
            function (r) {
              return r.data;
            },
      ...
      function addFolderInit(folderID) {
        return $http.get(urlbase + "/config/defaults/folder").then(function (response) {
          ...
          initShareEditing("folder");
          $scope.currentSharing.unrelated = $scope.currentSharing.unrelated.concat(
            $scope.currentSharing.shared
          );
          $scope.currentSharing.shared = [];
      ...
      $scope.shareFolderWithDevice = function (folder, device) {
        ...
        if (
          folderCfg.type == "receiveencrypted" ||
          !$scope.pendingIsRemoteEncrypted(folder, device)
        ) {
        ...
        switch (folderCfg._guiVersioning.selector) {
          ...
          case "simple":
            (folderCfg.versioning.params.keep = "" + folderCfg._guiVersioning.simpleKeep),
              (folderCfg.versioning.params.cleanoutDays =
                "" + folderCfg._guiVersioning.trashcanClean);
            break;
          case "staggered":
            folderCfg.versioning.params.maxAge =
              "" + folderCfg._guiVersioning.staggeredMaxAge * 86400;
            break;
          case "external":
          ...
          massAction: function (name, action) {
            $.each($scope.restoreVersions.versions, function (key) {
              if (
                ...
                $.each(
                  $scope.restoreVersions.filterVersions($scope.restoreVersions.versions[key]),
          ...
          show: function (folder) {
            ...
            $q.all([dataReceived, modalShown.promise]).then(function () {
              $timeout(function () {
                ...
                $scope.restoreVersions.tree = $("#restoreTree")
                  .fancytree({
                    ...
                    renderColumns: function (event, data) {
                      ...
                      if (node.folder) {
                        template =
                          "<div ng-include=\"'syncthing/folder/restoreVersionsMassActions.html'\"/>";
                      } else {
                        template =
                          "<div ng-include=\"'syncthing/folder/restoreVersionsVersionSelector.html'\"/>";
                      }
                ...
                ranges[$translate.instant("Yesterday")] = [
                  moment().subtract(1, "days").startOf("day"),
                  moment().startOf("day"),
                ];
                ranges[$translate.instant("Last 7 Days")] = [
                  moment().subtract(6, "days").startOf("day"),
                  moment(),
                ];
                ranges[$translate.instant("Last 30 Days")] = [
                  moment().subtract(29, "days").startOf("day"),
                  moment(),
                ];
                ranges[$translate.instant("This Month")] = [moment().startOf("month"), moment()];
                ranges[$translate.instant("Last Month")] = [
      ...
      $scope.revertOverride = function () {
        $http.post(
          urlbase +
            "/db/" +
            $scope.revertOverrideParams.operation +
            "?folder=" +
      ...
      $scope.shareDeviceIdDialog = function (method) {
        ...
        var title = $translate.instant('Syncthing device ID for "{%devicename%}"', {
          devicename: deviceName,
        });
        var footer = $translate.instant("Learn more at {%url%}", { url: "https://sync86.com" });
      ...
      $scope.shareDeviceId = function () {
        switch ($scope.shareDeviceIdParams.method) {
          case "email":
            location.href =
              "mailto:?subject=" +
              encodeURIComponent($scope.shareDeviceIdParams.subject) +
              "&body=" +
              encodeURIComponent($scope.shareDeviceIdParams.body);
            break;
          case "sms":
      ...
      $scope.removeXattrEntry = function (entry) {
        $scope.currentFolder.xattrFilter.entries = $scope.currentFolder.xattrFilter.entries.filter(
          function (n) {
            return n !== entry;
          }
        );
